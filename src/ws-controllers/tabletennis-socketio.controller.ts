...
export class TabletennisSocketioController {
  constructor(
    ...
    @service(TabletennisSocketStoreService) private socketStore: TabletennisSocketStoreService
  ) {
  }
  ...
  async connect(socket: TableTennisSocket) {
    if (socket.handshake.auth.tabletennis) {
      (socket as TableTennisSocket).tabletennis = socket.handshake.auth.tabletennis
  ...
  async update(gameChange: TableTennisGame) {
    try {
      ...
      const newState = await this.tabletennisService.getById(this.socket.tabletennis)
      this.socketStore.sendToAll(this.socket.tabletennis, TabletennisSocketPaths.tabletennisUpdate, newState)
    } catch (error) {
      console.log(error);
